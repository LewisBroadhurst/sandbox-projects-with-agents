...
function CityStats({ state }: { state: GameState }) {
  ...
  const emp =
    jobs > 0 ? Math.min(100, Math.round((state.population / jobs) * 100)) : 100;
...
function TileInfo({
  ...
  selectedTile,
  onDemolish,
}: Omit<SidePanelProps, 'onBlessing'>) {
  if (selectedBuild) {
  ...
  if (!selectedTile) {
    return (
      <div id="tileInfo">
        <div className="empty">
          Click a tile to inspect it, or choose a building from the left to
          place it.
        </div>
      </div>
    );
      ...
      {b ? (
        <>
          ...
          <button
            ...
            onClick={() => onDemolish(selectedTile.x, selectedTile.y)}
          >
            🏚️ Demolish (refund 50%)
          </button>
        </>
      ) : (
...
function Blessings({
  state,
  onBlessing,
}: Pick<SidePanelProps, 'state' | 'onBlessing'>) {
      ...
      {ids.map((id) => {
        ...
        return (
          <button
            ...
            disabled={
              state.resources.favor < bl.cost || (bl.dur > 0 && active)
            }
  ...
  return (
    <div id="milestoneList">
      {MILESTONES.map((m) => {
        ...
        const rewardStr = (Object.entries(m.reward) as [ResourceKey, number][])
          .map(([k, v]) => `${RES_META[k].icon}${v}`)
          .join(' ');
        return (
