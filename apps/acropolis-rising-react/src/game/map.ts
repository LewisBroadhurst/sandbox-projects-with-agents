...
export function generateMap(rng: Rng): Tile[] {
	const map: Tile[] = [];
	for (let y = 0; y < ROWS; y++) for (let x = 0; x < COLS; x++) map.push({ x, y, terrain: 'grass', building: null });
	const at = (x: number, y: number) => map[y * COLS + x];

	// carve coastline on the right edge, jagged
	for (let y = 0; y < ROWS; y++) {
		const depth = 3 + Math.floor(Math.sin(y * 0.7) * 1.5) + Math.floor(rng.next() * 2);
		...
		for (let x = 0; x < COLS; x++) {
			...
			if (t.terrain === 'grass') {
				...
				for (const [dx, dy] of neighbors) {
					const nx = x + dx,
						ny = y + dy;
					if (nx >= 0 && nx < COLS && ny >= 0 && ny < ROWS && at(nx, ny).terrain === 'water') {
						t.terrain = 'coast';
						break;
					}
				}
