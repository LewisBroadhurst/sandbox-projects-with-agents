...
const hasAllResources = (v: unknown): boolean => isObject(v) && RESOURCE_ORDER.every(k => isFiniteNumber(v[k]));
...
function isValidGameState(x: unknown): x is GameState {
	return isObject(x) && Object.entries(STATE_SCHEMA).every(([key, ok]) => ok(x[key]));
}
