...
export function useGame() {
  ...
  useEffect(() => {
    ...
    pushToast(
      loadedFromSave.current
        ? `Welcome back to ${stateRef.current.cityName}.`
  ...
  const demolish = useCallback(
    (x: number, y: number) => apply((s) => bulldoze(s, x, y)),
    [apply]
  );

  const invokeBlessing = useCallback(
    (id: BlessingId) => apply((s) => castBlessing(s, id)),
    [apply]
  );
  ...
  const newCity = useCallback(() => {
    if (
      !window.confirm(
        'Start a new city? This will not erase your saved game unless you save over it.'
      )
    )
      return;
