...
export function App() {
	...
	const coverage = useMemo(() => computeCoverage(game.state.map, AGORA_RANGE), [game.state.map]);
	const storage = useMemo(() => computeStorageAccess(game.state.map, STOREHOUSE_RANGE), [game.state.map]);
	const cartRoutes = useMemo(
		() => [...computeCartRoutes(game.state.map, AGORA_RANGE), ...computeGoodsRoutes(game.state.map, STOREHOUSE_RANGE)],
		[game.state.map],
	);
	...
	return (
		<div id="app">
			...
			/>
			<div id="main">
				<Palette selectedBuild={game.selectedBuild} onSelect={game.selectBuild} />
				<GameCanvas
					map={game.state.map}
